...
if not st.session_state.logs_initial_load_done or filters_changed:
    query_obj = session.query(EvRkProcDqApex)
    if filters:
        query_obj = query_obj.filter(*filters)
    query_obj = query_obj.order_by(EvRkProcDqApex.processing_date.desc()).offset(offset).limit(limit)
    # Print the SQL generated by SQLAlchemy
    st.write(f"DEBUG SQL: {str(query_obj.statement.compile(dialect=oracle.dialect(), compile_kwargs={'literal_binds': True}))}")
    results = query_obj.all()
    # Fix: If results are ORM objects, use __dict__, else use as-is
    if results and hasattr(results[0], '__table__'):
        # ORM object: use __dict__
        new_data = pd.DataFrame([{k: v for k, v in r.__dict__.items() if not k.startswith('_')} for r in results])
    else:
